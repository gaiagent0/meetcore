...
class DatabaseManager:
    ...
    async def save_meeting(self, meeting_id: str, title: str, folder_path: str = None):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Check if meeting exists by ID only (title can repeat)
                ...
                if not existing_meeting:
                    ...
                    logger.info(f"Saved meeting {meeting_id}")
                else:
    ...
    async def save_meeting_transcript(self, meeting_id: str, transcript: str, timestamp: str,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (str(_uuid.uuid4()), meeting_id, transcript, timestamp, summary, action_items, key_points,
                      audio_start_time, audio_end_time, duration))
    ...
    async def get_meeting(self, meeting_id: str):
        """Get a meeting by ID with all its transcripts and summary"""
        try:
            async with self._get_connection() as conn:
                # Get meeting details
                ...
                # Get summary from summary_processes
                summary_data = None
                cursor = await conn.execute("""
                    SELECT result, metadata, created_at
                    FROM summary_processes
                    WHERE meeting_id = ? AND status = 'COMPLETED' AND result IS NOT NULL
                """, (meeting_id,))
                sp_row = await cursor.fetchone()
                if sp_row and sp_row[0]:
                    try:
                        metadata = json.loads(sp_row[1]) if sp_row[1] else {}
                        summary_data = {
                            'summary_json': sp_row[0],
                ...
                # Also try transcript_chunks for transcript if transcripts table is empty
                ...
                # If transcripts table empty but transcript_chunks has data, expose it
                if not transcripts and full_transcript:
                    result['transcripts'] = [{
                        'id': meeting_id,
    ...
    def _api_key_column(self, provider: str) -> str:
        """Provider neve → DB oszlop neve."""
        mapping = {
            "openai":      "openaiApiKey",
    ...
    async def save_api_key(self, api_key: str, provider: str):
        """Save the API key"""
        api_key_name = self._api_key_column(provider)
    ...
    async def get_api_key(self, provider: str) -> str:
        """Get the API key for a provider. Returns '' if not set."""
        try:
            api_key_name = self._api_key_column(provider)
        except ValueError:
            return ""
        try:
            async with self._get_connection() as conn:
                # openrouterApiKey oszlop lehet hogy régi DB-ben még nincs — ADD COLUMN ha hiányzik
                try:
                    await conn.execute(f"ALTER TABLE settings ADD COLUMN {api_key_name} TEXT")
                    await conn.commit()
                except Exception:
                    pass  # már létezik
            ...
            logger.warning(f"get_api_key({provider}): {e}")
            return ""

    async def clear_api_key(self, provider: str) -> None:
    ...
    async def get_all_api_keys_status(self) -> dict:
        """Returns dict: provider → bool (has key configured)."""
        providers = ["claude", "groq", "openai", "openrouter"]
        result = {}
        for p in providers:
            key = await self.get_api_key(p)
            result[p] = bool(key)
        return result
